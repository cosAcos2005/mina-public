...
- **ORM原理: 出力次元がスケール往復の必要性を決める**（2026-05-12 C型転移）[life-os]: SNPの適用条件節。出力が全スケールを映す高解像度テンソル（セグメンテーション・深度推定・超解像）= SNP必要。出力が低次元スカラー/ベクトル（分類・回帰）= SNP不要（ただしSNPが「より重要でなくなる」程度の主張——不要でも恩恵はある）。確認事例: U-Net vs ViT（セグメンテーション vs 分類）でhit_rate 5/6 ≈ 83%。P-ORM-01「U-Net→ResNet変更で境界精度低下」✓ / P-ORM-02「UNet-like Transformer > ViT on segmentation」✓（Swin-UNet 2021）。⚠️限界: MAE事前学習など「学習タスク出力次元 ≠ 推論タスク出力次元」のケースでは適用条件が不明確になる → [[connections/2026-05-12_orm-principle-snp-applicability.md]]
