...
| **強制代替の結晶化: 安いデフォルトを除去すると高Fisher路が義務化され反復が能力を不可逆に固定する**（2026-07-16 Session 22） | 彫刻の原理（空間相: 削って構造が顕現）の動力学的拡張（時間相: 顕現した構造が反復でPNN固定）。安い認知路（ミニマップ、左右語彙、チュートリアル）除去→高コスト路（空間認知、絶対方向演算、法則発見）義務化→義務的反復がPNN凝縮を経て不可逆化。ゲーム（ミニマップ剥奪）/ 音楽（ブルーノート/十二音）/ 建築（にじり口）/ 教育（Bjork望ましい困難）で多ドメイン確認。設計原則: 「学ばせたい能力Xの安価な代替Yを除去せよ」→ [[threads/2026-07-16_vocabulary-bandwidth-crystallization-stype.md]] |
| **結晶化はPNN凝縮による学習の不可逆化だ — 義務的反復は学ぶだけでなく消えなくする**（2026-07-16 Session 22） | 「義務化→結晶化」の神経科学的機構の特定。LTP（学習そのもの: 可塑性発現）≠ PNN（学習の保護: 可塑性終了）。正確な機構鎖: 義務的反復→回路活動↑→PV陽性介在ニューロン成熟+Otx2集積→PNN凝縮→臨界期閉鎖→消去耐性獲得=結晶化。Hensch 2004（暗室飼育でPNN成熟遅延）/ Gogolla 2009（PNN後の恐怖記憶消去困難）が支持。結晶化ステップ=Layer 0 PNN化=CLHPC内面化完了 → [[threads/2026-07-16_vocabulary-bandwidth-crystallization-stype.md]] |
| **除去するデフォルトの選択が結晶化する能力の選択だ — PNNは価値中立で悪習も固定する**（2026-07-16 Session 22） | 強制代替の結晶化の設計的罠。PNNは「望ましい能力」も「誤パターン・悪習」も同じ機構で固定する。結晶化は価値中立。「何を削るか」の選択=「何を育てるか」の選択（等価）。適用限界4条件: (1)高コスト代替が実在可達、(2)行為者に代替到達能力あり、(3)結晶化する型が望ましい、(4)除去対象が安全必須でなく利便デフォルト。追加第5条件: 除去のタイミングが臨界期と同期。Bjork「望ましい困難」の条件節と同型 → [[threads/2026-07-16_forced-substitution-crystallization-ctype.md]] |
